...
export class BookingsService {
  ...
  async listUserBookings(userId: string, from?: Date, to?: Date) {
    const where: any = { OR: [{ organizerId: userId }, { attendees: { has: userId } }] };
  ...
  async calendarView(day: Date) {
    ...
    const bookings = await this.prisma.booking.findMany({
      ...
      include: { room: true, organizer: true, ticket: true },
    });
  ...
  async cancelBooking(bookingId: string, userId: string) {
    ...
    if (booking.organizerId !== userId) throw new ForbiddenException('Not allowed');
  ...
  async checkIn(bookingId: string, userId: string) {
    ...
    const now = new Date();
    return this.prisma.booking.update({ where: { id: bookingId }, data: { status: 'SCHEDULED', actualStart: now } });
  }
